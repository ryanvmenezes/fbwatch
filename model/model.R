...
goal.pred = glm(
  goals.left ~ clean.minute + clean.minute:goals.edge + spi.edge + clean.minute*men.edge + home,
  data = train,
  family = 'poisson'
)
...
calcs = test %>% 
  mutate(pred.goals.left = predict(goal.pred, newdata = ., type = 'response')) %>% 
  ...
  mutate(
    plot = pmap(
      ...
      ~..1 %>%
        ...
        ggplot(aes(x = clean.minute, y = prob, fill = side)) +
        geom_area() +
        # geom_line() +
        labs(
